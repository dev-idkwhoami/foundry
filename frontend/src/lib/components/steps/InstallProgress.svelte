			...
			<Card.Content class="flex-1 min-h-0 flex flex-col">
				...
				>
					{#each logs as log, i (i)}
						<div class="leading-5">
							{#if log.level === "error"}
								<span class="text-red-400">{@html ansi.ansi_to_html(log.message)}</span>
							{:else if log.level === "warn"}
								<span class="text-amber-400">{@html ansi.ansi_to_html(log.message)}</span>
							{:else if log.level === "success"}
								<span class="text-green-400">{@html ansi.ansi_to_html(log.message)}</span>
							{:else}
								{@html ansi.ansi_to_html(log.message)}
							{/if}
						</div>
					{/each}
					{#if logs.length === 0}
